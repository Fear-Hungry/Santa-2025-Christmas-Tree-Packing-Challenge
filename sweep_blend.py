...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "Ex.: \"./solver_tile --k 4 --seed {seed} --shift {shift_a},{shift_b} --output {out}\""
        ),
    )
    ap.add_argument(
        "--cmd-mode",
    ...
    ensemble_cmd = ["./ensemble_submissions", args.out, *run_paths]
    if args.no_final_rigid:
        ensemble_cmd.append("--no-final-rigid")

    subprocess.run(ensemble_cmd, check=True)
    subprocess.run(["./score_submission", args.out], check=True)
    return 0
