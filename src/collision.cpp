...
bool any_overlap(const Polygon& base_poly,
    ...
    auto aabb_overlap = [&](const BoundingBox& a, const BoundingBox& b) -> bool {
        if (a.max_x < b.min_x - eps || b.max_x < a.min_x - eps) {
            return false;
        }
        if (a.max_y < b.min_y - eps || b.max_y < a.min_y - eps) {
            return false;
        }
        return true;
    };
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j : neigh) {
            ...
            if (polygons_intersect_impl(polys[static_cast<size_t>(i)],
